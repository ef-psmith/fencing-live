...
<topdiv name="topdiv" id="vlistid" class="vlist_entry">
	...
	<content>
		...
		<div class="vlist_title" id="vtitle">
			<h2>Entry List for <xsl:value-of select="../@titre_ligne" /> - <xsl:value-of select="@present" /> present / <xsl:value-of select="@entries" /> entered</h2>
			<div class="pres_key">
...
<span>
	<xsl:attribute name="class">
		...
		<xsl:choose>
			...
			<xsl:otherwise>pres_abs</xsl:otherwise>
		</xsl:choose>

	</xsl:attribute>

	<span class="col_name"><xsl:value-of select="@name" /></span>
	<span class="col_club"><xsl:value-of select="@affiliation" />
...
<!-- this seems to break frequently - closing span tags get missed off -->
...
<xsl:template match="lists[@name='ranking' and 
   ( 
	  (
		 (
			( starts-with(../@stage, 'poules') and contains(../@stage, 'finished'))
		 	or 
		 	( starts-with(../@stage, 'tableau') and ( 1 &gt;= count(../lists[@name='ranking' and @type='final' ]/fencer[@elimround != 'elim_p'])))
		) 
		and @type='pools'
	  ) 
	  or 
	  (
		 (
			...
			(
				...
				and not 
				( 1 &gt;= count(../lists[@name='ranking' and @type='final' ]/fencer[@elimround != 'elim_p']))
			) 
		 )
		 and 
		 @type='final'
...
<xsl:variable name="col1matchcount" select="tableau[@name = $col1]/@count" />
<xsl:variable name="col2name" select="concat(substring($col1, 1, 1), $col1matchcount)" />
<xsl:variable name="col2" select="tableau[@name = $col2name]/@name" />
