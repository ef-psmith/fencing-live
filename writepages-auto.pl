	...
	foreach my $tabwhere (@tableaux)
	{
		...
		while ($preceedingbout < $roundsize / 2) 
		{
			...
			if (0 == $chosenpart || $preceedingbout == ($minroundsize /2) * $chosenpart) 
			{
				...
				# my $part = ($defindex + 1) unless $secondtableau;;
				...
				my $divname = "T" . $defindex;
				my $title_id = "TT" . $defindex;
				...
				# $extratitle = " Repecharge" if $tab->is_rep;
				...
				# $extratitle = " " . $tab->nom_etendu . $extratitle;

				$def{'tableau_title'} = $compname . "<br>" . $tab->nom_etendu;
				$def{'tableau_title'} .= " part " . $part if $roundsize > $minroundsize;

				#if ($preceedingbout == 0 && $roundsize <= 8 && !$tab->is_rep) 
				#{
